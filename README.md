...
## Длинные документы и память (flash-attn — опционально)

HF-путь по умолчанию использует eager-внимание O(n²). На **инвойсах (1–3 страницы) это не
проблема** — короткая последовательность спокойно влезает в 24 GB. OOM возникает только на
**длинных многостраничных** PDF.

Поэтому flash-attn по умолчанию **НЕ ставится** (образ собирается быстро и заводится на любой
NVIDIA-карте), а модель грузится в eager (`UOCR_ATTN` пустой). У модели нет `sdpa`-пути
(`ATTENTION_CLASSES` = eager / flash_attention_2 / sliding-window), поэтому единственная
in-process альтернатива eager — это `flash_attention_2`.

**Включить flash-attn (только если реально нужны длинные доки в этом процессе):**

```bash
# ВАЖНО: арку под свою карту -> 4090 (Ada)=8.9, 5090 (Blackwell)=12.0+PTX
INSTALL_FLASH_ATTN=1 CUDA_ARCH=8.9 docker compose build
UOCR_ATTN=flash_attention_2 docker compose up -d
```

- **Сборка компилирует flash-attn из исходников** (готового wheel под torch 2.10/cu129 пока
  нет): долго, ест RAM (`MAX_JOBS=4` × неск. ГБ) и диск. На сервере с ≤32 GB RAM снизь
  `MAX_JOBS` до 2. Убедись, что на диске десятки ГБ под билд-дерево.
- Неправильная арка = `no kernel image for sm_XX` при инференсе (напр. образ под `12.0+PTX`
  не запустится на 4090/sm_89).

**Рекомендация для длинных документов — не flash-attn, а SGLang (вариант B).** Вендор кладёт в
репозиторий модели готовый `wheel/sglang-...whl`; SGLang приносит свой backend внимания
(flashinfer/fa3) с prebuilt-ядрами (в т.ч. под sm_89), так что руками ничего компилировать не
надо. См. `scripts/serve_sglang.sh`.
