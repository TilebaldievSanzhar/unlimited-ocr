...
## Длинные документы и память (flash-attn)

HF-путь по умолчанию использует eager-внимание O(n²), которое OOM'ит на длинных
многостраничных PDF. Поэтому в образ встроен **flash-attn**, и модель грузится с
`attn_implementation=flash_attention_2` (переменная `UOCR_ATTN`, по умолчанию включена) —
полная матрица внимания не материализуется.

- **Сборка flash-attn долгая** (на Blackwell/sm_120 — компиляция из исходников). Арка
  ограничена `TORCH_CUDA_ARCH_LIST=12.0+PTX`, параллелизм — `MAX_JOBS=4` (подними, если на
  билд-сервере много RAM).
- **Откат**, если flash-attn капризничает на этой GPU: `UOCR_ATTN=eager` (вернёт прежнее
  поведение; длинные доки снова будут OOM'ить — годится только для коротких/одностраничных).
- Это промежуточный путь (вариант A). Штатный прод-путь модели — SGLang (`scripts/serve_sglang.sh`,
  `--attention-backend fa3`/flashinfer), вариант B.
